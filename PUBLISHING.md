...
- **Maven coordinates** are repo-specific (`io.github.OWNER.REPO_NAME` and `chat-library-poc-android`) to avoid GitHub Packages 422 errors when the same coordinates were used by another repo.
- **Maven URL consumers use:**  
  `https://maven.pkg.github.com/YOUR_GITHUB_OWNER/chat-library-poc`
- **Dependency coordinates:**  
  `io.github.prasadpatil1508.chat_library_poc:chat-library-poc-android:VERSION`  
  (e.g. `io.github.prasadpatil1508.chat_library_poc:chat-library-poc-android:1.0.0`).
...
**If you get "422 Unprocessable Entity":** Common causes:
- **Version already exists** – That version was already published (e.g. same tag pushed twice or workflow re-run). Use a **new version** (e.g. `v1.0.26` or `-PLIB_VERSION=1.0.26`) and never re-publish the same version.
- **artifactId must be lowercase** – The project sets `artifactId = "chat-library-poc-android"` for the Android publication; if you change it, keep it lowercase.
- **Package tied to another repo** – If the same `groupId:artifactId` was ever published from a different GitHub repo, GitHub may reject. Delete the package from the wrong repo (Packages → package → Package settings → Delete) or use a different `groupId`/`artifactId`.
...
- [ ] **Android:** Library published to GitHub Packages; app has the GitHub Packages repo and `implementation("io.github.prasadpatil1508.chat_library_poc:chat-library-poc-android:VERSION")`; credentials set via `gpr.user`/`gpr.token` or `GITHUB_ACTOR`/`GITHUB_TOKEN`.
