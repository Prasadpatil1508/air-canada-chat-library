...
**"chat-library-poc-android (AAR) missing" or "--- io.github...chat-library-poc-android:1.0.0 FAILED":**
- The library should publish `chat-library-poc-android` automatically
- Try: `./gradlew :shared:clean :shared:publishToMavenLocal`
- Check `~/.m2/repository/io/github/prasadpatil1508/chat_library_poc/chat-library-poc-android/1.0.0/` exists
...
5. **If using Maven Local:** After republishing the library, you can delete the cached copy so the host must re-fetch: remove `~/.m2/repository/io/github/prasadpatil1508/chat_library_poc/` (or only the `chat-library-poc-android/` subfolder for the version you use), then sync/rebuild the host.
...
**"Cannot select this directory" when adding the package locally**
- Do **not** use **File → Add Package Dependencies** and then "Add Local..." with the library repo folder. This repo’s `Package.swift` is for **remote** use only (it points at a GitHub Release zip). Xcode does not support using it as a local Swift package.
- For **local** use: build the XCFramework (step 1 above), then add the **built** `ChatSDK.xcframework` via **Add Other… → Add Files…** (step 2). You are adding the framework file/folder, not the repo as a package.
